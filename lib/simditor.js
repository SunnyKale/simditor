(function() {
  ...
  ImageButton = (function(_super) {
    ...
    ImageButton.prototype.loadImage = function($img, src, callback) {
      ...
      img.onload = function() {
        var imageSize;
        imageSize = _this.limitImage(img.width, img.height);
        $img.attr({
          src: src,
        ...
        $wrapper.width(imageSize.width).height(imageSize.height);
    ...
    ImageButton.prototype.resizeImage = function($target, width, height) {
      var $img, imageSize;
      $img = $target.find('img');
      imageSize = this.limitImage(width, height);
      $img.attr({
        width: imageSize.width,
        height: imageSize.height
      });
      ...
      this.popover.widthEl.val(imageSize.width);
      return this.popover.heightEl.val(imageSize.height);
    };

    ImageButton.prototype.limitImage = function(width, height) {
  ...
  ImagePopover = (function(_super) {
    ...
    ImagePopover.prototype.render = function() {
      ...
      this.el.find('input:text').on('keydown', function(e) {
      ...
      this.widthEl.on('input propertychange', function(e) {
        var $img, height, width;
        $img = _this.target.find('img');
        width = parseInt(_this.widthEl.val() || 0);
        if (typeof width === 'number') {
          height = width * $img.height() / $img.width();
          return _this.button.resizeImage(_this.target, width, height);
        }
      });
      this.heightEl.on('input propertychange', function(e) {
        var $img, height, width;
        $img = _this.target.find('img');
        height = parseInt(_this.heightEl.val() || 0);
    ...
    ImagePopover.prototype._initUploader = function() {
      ...
      this.editor.uploader.on('uploadsuccess', function(e, file, result) {
        ...
        return _this.button.loadImage($img, result.file_path, function() {
          ...
          _this.widthEl.val($img.width());
          _this.heightEl.val($img.height());
          return _this.editor.trigger('valuechanged');
        });
      });
    ...
    ImagePopover.prototype.show = function() {
      ...
      this.widthEl.val($img.width());
      return this.heightEl.val($img.height());
    };
  ...
  TableButton = (function(_super) {
    ...
    function TableButton() {
      ...
      $.merge(this.editor.formatter._allowedTags.push, ['tbody', 'tr', 'td', 'colgroup', 'col']);
    ...
    TableButton.prototype.initResize = function($table) {
      ...
      $resizeHandle = $('<div class="resize-handle"></div>').appendTo($wrapper);
      $wrapper.on('mouseenter', 'td', function(e) {
        var $col, $td, index;
        ...
        return $resizeHandle.css('left', $td.position().left + $td.outerWidth() - 2).data('td', $td).data('col', $col).show();
      });
      $wrapper.on('mouseleave', function(e) {
        return $resizeHandle.hide();
      });
      return $wrapper.on('mousedown', '.resize-handle', function(e) {
        var $col, $handle, $td, startLeft, startWidth, startX;
        ...
        $(document).on('mousemove.simditor-resize-table', function(e) {
          ...
          return $handle.css('left', startLeft + deltaX);
        });
